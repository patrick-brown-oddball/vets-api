...
module MedicalRecords
  ...
  class LighthouseClient < Common::Client::Base
    ...
    DIAGNOSTIC_STUDY_NOTE = '70004-7' # Diagnostic study note
    ...
    def list_vitals
      ...
      bundle = fhir_search(FHIR::Observation, search: { parameters: { patient: @icn, code: loinc_codes } })
      sort_bundle(bundle, :effectiveDateTime, :desc)
    end

    def list_conditions
      bundle = fhir_search(FHIR::Condition, search: { parameters: { patient: @icn } })
      sort_bundle(bundle, :recordedDate, :desc)
    end
    ...
    def list_clinical_notes
      loinc_codes = "#{PHYSICIAN_PROCEDURE_NOTE},#{DISCHARGE_SUMMARY},#{DIAGNOSTIC_STUDY_NOTE}"
      bundle = fhir_search(FHIR::DocumentReference,
                           {
                             search: { parameters: { patient: @icn, type: loinc_codes } },
                             headers: { 'Cache-Control': 'no-cache' }
                           })
    ...
    def list_labs_chemhem_diagnostic_report
      fhir_search(FHIR::DiagnosticReport,
                  search: { parameters: { patient: @icn, category: 'LAB' } })
    end
    ...
    def list_labs_other_diagnostic_report
      ...
      fhir_search(FHIR::DiagnosticReport, search: { parameters: { patient: @icn, code: loinc_codes } })
    end
